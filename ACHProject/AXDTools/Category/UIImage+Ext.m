...
+(UIImage *)imageWithColor:(UIColor *)aColor size:(CGSize)size{
    ...
    return [UIImage imageWithColor:aColor withFrame:CGRectMake(0, 0, size.width, size.height)];
}
...
+ (NSData *)zipImageWithImage:(UIImage *)image
{
    ...
    while ([compressedData length] > maxFileSize) {
        compression *= 0.9;
        image = [[self class] compressImage:image newWidth:image.size.width*compression];
        compressedData = UIImageJPEGRepresentation(image, compression);
    }
    return compressedData;
}

//+ (NSData *)zipImageWithImage:(UIImage *)image targetbytes:(const void *)bytes{
//    if (!image) {
//        return nil;
//    }
//    
